...
for n_ds, ds in enumerate(datasets):
    ...
    params = [preprocessing, data_transformation, as_techniques, ml_techniques, pt_techniques]
    flattened_params = [list(x) for x in itertools.product(*params)]
    # Cross validation
    for iteration, (train_index, test_index) in enumerate(cv.split(dataframe)):        
        
        for ppt, dtt, ast, mlt, pst in (pbar:= tqdm(flattened_params, total=len(flattened_params))):
            astp = ast.parameters
            current_time = datetime.datetime.now()
            pbar.set_description(current_time.strftime("%H:%M:%S") + " : "f"PT: {pst.name}")
            pbar.set_postfix(batch=f"DT: {dtt.name}" + " | " + f"AS: {ast.name}" + " | " + f"ML: {mlt.name}" + " | " +f"PP: {ppt.name}" + " | " + f"Dataset: {ds.name}")
